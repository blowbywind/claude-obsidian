...
- **Next.js 16 Build Adapters API (alpha)**: 비Vercel 환경 배포를 원활하게 지원하는 빌드 어댑터 규격(experimental alpha)이 도입되었으며, `AGENTS.md` 자동 생성 및 브라우저 로그 포워딩 등 AI 에이전트용 개발도구 기능이 강화되었습니다.
...
## 프로필 반영 후보 (저위험)
- Build Adapters API: Next.js 16 (alpha) 기반 비Vercel 플랫폼 배포 환경 문서 수집 시 필수 검증 규격으로 반영
