...
## 4. 모바일 홈화면(PWA) 앱 아이콘 제작 및 구축 사례 (2026-06-24 추가)

모바일 환경(특히 iOS 홈화면 등록 등)에서 웹앱(PWA)으로 등록될 때 고해상도로 로드되는 브랜딩 아이콘을 직접 구축 및 적용한 실무 사례입니다.

### ① 고해상도 앱 아이콘 이미지 에셋 생성
- **콘셉트**: AI 자동화 플랫폼 "Autobots"의 성격을 반영하여 메탈릭 질감의 알파벳 'A'와 사이버네틱 라인이 흐르는 미래지향적 심볼 디자인.
- **색상**: 딥 바이올렛(Deep Violet), 다크 인디고(Dark Indigo)에 네온 일렉트릭 핑크(Electric Pink) 포인트를 주는 프리미엄 UI 그라데이션 백그라운드.
- **여백 설계(Safe Zone)**: iOS/Android 기기별 둥근 모서리 마스킹 시 로고가 잘리지 않도록 중앙 60% 영역 내에 핵심 엠블럼을 배치하는 마스크가능(Maskable) 안전 마진 유지.

### ② 해상도별 정밀 리사이징 및 배치 자동화
- **사용 도구**: 순수 JavaScript 기반 이미지 조작 패키지인 `jimp`를 사용해 의존성을 제어하고, 고화질 픽셀 보간법을 적용해 2가지 표준 해상도로 출력.
  - `public/icon-192.png`: PWA 숏컷 및 모바일 퀵 링크용 (192x192px, PNG)
  - `public/icon-512.png`: 고해상도 홈화면 등록 및 스플래시 화면용 (512x512px, PNG)
- **HTML/JSON 링크 매핑**:
  - `manifest.json` 내 `icons` 프로퍼티에 마스크가능 목적(`"purpose": "any maskable"`)으로 파일 연결.
  - `layout.tsx` 내 `<head>` 영역에 `<link rel="apple-touch-icon" href="/autobots/icon-192.png" />` 수동 선언을 동반하여 iOS Safari 홈화면 추가 인터랙션 최적화.

### ③ 빌드 통합 및 배포 정합성 확보
- 정적 자산이 최종 번들 디렉터리(`out/`)에 누락 없이 복사되도록 `npm run build`를 수행하여, Next.js의 최종 컴파일과 정적 HTML/Asset 내보내기 검증을 일치시킴으로써 실시간 배포 경로의 데이터 정합성을 확보함.

## 관련 노트
- [[wcag-2-2-새-성공-기준-요약]] — 1절 ⑤ 터치 타깃 최소 크기(WCAG 2.2) 기준의 근거
- [[tailwind-css-v4]] — 3절 Tailwind CSS 환경 아이콘 연동의 배경 스택
